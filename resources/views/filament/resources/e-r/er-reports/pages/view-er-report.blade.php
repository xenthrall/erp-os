<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <x-filament::section icon="heroicon-o-tag" heading="Clasificación del Evento: {{ $record->type->name ?? 'N/A' }}">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">

                {{-- Descripción del tipo de falta --}}
                <div class="md:col-span-2 lg:col-span-3">
                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Descripción de la Política / Falta</span>
                    ...
                    @foreach($record->attachments as $attachment)
                        <a href="{{ Storage::disk('public')->url($attachment->path) }}" target="_blank" 
                           class="flex flex-col items-center justify-center p-4 border rounded-xl hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800 transition shadow-sm">
                            ...
                            <span class="text-xs text-center truncate w-full text-gray-600 dark:text-gray-300" title="{{ basename($attachment->path) }}">
        ...
        <x-filament::section icon="heroicon-o-clipboard-document-list" heading="Bitácora de Observaciones">
                    ...
                    @foreach($record->notes->sortByDesc('created_at') as $note)
                        <div class="flex gap-x-3">
                            ...
                            <div class="relative last:after:hidden after:absolute after:top-7 after:bottom-0 after:start-3.5 after:w-px after:-translate-x-[0.5px] after:bg-gray-200 dark:after:bg-gray-700">
                                <div class="relative z-10 flex size-7 items-center justify-center rounded-full bg-primary-50 dark:bg-primary-900/50 ring-2 ring-white dark:ring-gray-900 shadow-sm">
                                    <x-filament::icon icon="heroicon-o-user" class="size-4 text-primary-600 dark:text-primary-400" />
                                </div>
                            </div>
                            ...
                            <div class="grow pt-0.5 pb-6">
                                <div class="flex justify-between items-center mb-1">
                                    ...
                                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400" title="{{ $note->created_at->format('d/m/Y h:i A') }}">
                                        {{ $note->created_at->diffForHumans() }}
                                    </span>
                                </div>
                                
                                {{-- 👇 Aquí agregamos "break-words" para evitar que el texto se desborde --}}
                                <div class="mt-2 text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/50 p-3.5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm break-words">
                                    {!! nl2br(e($note->note)) !!}
                                </div>
            ...
            @else
                <div class="flex flex-col items-center justify-center p-6 text-gray-500 dark:text-gray-400">
                    <x-filament::icon icon="heroicon-o-chat-bubble-left-right" class="w-10 h-10 mb-3 text-gray-300 dark:text-gray-600" />
