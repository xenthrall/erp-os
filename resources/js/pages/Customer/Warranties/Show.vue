        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-8 max-w-4xl mx-auto w-full">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <!-- Sección: Producto -->
                    <section class="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm p-5 md:p-6 space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ warranty.model }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mb-1">Cantidad</p>
                                <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ warranty.quantity }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mb-1 flex items-center gap-1">
                                    <Hash class="w-3 h-3" /> Número de Factura
                                </p>
                                <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ warranty.invoice_number }}</p>
                            </div>
                            <div v-if="warranty.internal_code">
                                <p class="text-xs text-slate-400 dark:text-slate-500 mb-1">Código Interno</p>
                                <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ warranty.internal_code }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mb-1 flex items-center gap-1">
                                    <Calendar class="w-3 h-3" /> Fecha de Compra
                                </p>
                                <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ formatDate(warranty.purchase_date) }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mb-1 flex items-center gap-1">
                                    <Calendar class="w-3 h-3" /> Fecha del Daño
                                </p>
                                <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ formatDate(warranty.damage_date) }}</p>
                            </div>
                        </div>
                    </section>

                    <!-- Sección: Descripción de la falla -->
                    <section class="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm p-5 md:p-6 space-y-3">
                        <h2 class="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                    ...
                    <!-- Sección: Notas del equipo (si existen) -->
                    <section
                        v-if="warranty.notes && warranty.notes.length > 0"
                    ...
                    >
                        <h2 class="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                            ...
                            Notas del Equipo
                            <span class="ml-auto text-xs font-normal normal-case text-slate-400">
                        ...
                        <ul class="space-y-3">
                            <li
                                v-for="note in warranty.notes"
                                :key="note.id"
                                class="bg-slate-50 dark:bg-zinc-800/50 rounded-xl p-4 space-y-1.5"
                            >
                                ...
                                <p class="text-sm text-slate-700 dark:text-slate-300 leading-relaxed pl-9">
                                    {{ note.note }}
                                </p>
                            </li>
                        </ul>
                    ...
                    <section
                        v-if="warranty.attachments && warranty.attachments.length > 0"
                    ...
                    >
                        ...
                        <ul class="space-y-2">
                            <li
                                v-for="file in warranty.attachments"
                                :key="file.id"
                ...
                <div class="space-y-6">
                    ...
                    <!-- Tarjeta: Resumen -->
                    <section class="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm p-5 space-y-4">
                        <h2 class="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                        ...
                        <dl class="space-y-3">
                            ...
                            <div class="flex justify-between items-start gap-2">
                                ...
                                <dd class="text-xs text-slate-600 dark:text-slate-400 text-right">{{ formatDate(warranty.created_at) }}</dd>
                            </div>
                            <div class="flex justify-between items-start gap-2">
                                <dt class="text-xs text-slate-400 dark:text-slate-500 shrink-0">Última actualización</dt>
                                <dd class="text-xs text-slate-600 dark:text-slate-400 text-right">{{ formatDate(warranty.updated_at) }}</dd>
                            </div>
                            <div v-if="warranty.factory_sequence" class="flex justify-between items-start gap-2">
                                ...
                                <dd class="text-sm font-semibold text-slate-800 dark:text-slate-200">{{ String(warranty.factory_sequence).padStart(4, '0') }}</dd>
                            </div>
                        </dl>
                    </section>
                    ...
                    <section class="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm p-5 space-y-4">
                        ...
                        <dl class="space-y-3">
                            <div>
                                ...
                                <dd class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ warranty.shipping_city }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs text-slate-400 dark:text-slate-500 mb-0.5">Dirección</dt>
                                <dd class="text-sm text-slate-700 dark:text-slate-300">{{ warranty.shipping_address }}</dd>
                            </div>
                        </dl>
                    </section>
                    ...
                    <!-- Acciones -->
                    <Link
                        v-if="warranty.status === 'pending'"
                        :href="`/customer/warranties/${warranty.id}/edit`"
